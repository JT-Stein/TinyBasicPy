...
class Tokenize:
  ...
  def code_generate(self, source):
    ...
    if "PLOT" in self.source:
      if not self.plot:
        self.plot = True

      self.points_extract = self.source.split(" ")[1].replace(" ", "").split(",")
      self.x = int(self.points_extract[0])
      self.y = int(self.points_extract[1])

      self.plot_x.append(self.x)
      self.plot_y.append(self.y)

    if self.source == "NEXT" and self.plot:
      print("Plotting...")
...
file_load = File()
user_code = file_load.read_file("graphic_test.bas")

if verbose:
  print(f"\nRead {file_load.file_name}: {user_code}")

if "PLOT" in user_code:
  print("PLOT IMPORTN")
